...
@jobController.post("/job/get", response_model=JobPageObjectResponse,
...
async def get_system_job_list(request: Request, job_page_query: JobPageObject, query_db: Session = Depends(get_db)):
    try:
        ...
        return MyResponse(data=notice_page_query_result, msg="获取成功")
    except Exception as e:
        logger.exception(e)
        return MyResponse(data="", msg=str(e))


@jobController.post("/job/add", response_model=CrudJobResponse,
                    dependencies=[Depends(CheckUserInterfaceAuth('monitor:job:add'))])
...
@jobController.patch("/job/edit", response_model=CrudJobResponse,
                     dependencies=[Depends(CheckUserInterfaceAuth('monitor:job:edit'))])
...
@jobController.post("/job/changeStatus", response_model=CrudJobResponse,
...
@jobController.post("/job/delete", response_model=CrudJobResponse,
...
@jobController.get("/job/{job_id}", response_model=JobModel,
...
async def export_system_job_list(request: Request, job_query: JobModel, query_db: Session = Depends(get_db)):
    try:
        ...
        return streaming_MyResponse(data=bytes2file_response(job_export_result))
    except Exception as e:
        logger.exception(e)
        return MyResponse(data="", msg=str(e))


@jobController.post("/jobLog/get", response_model=JobLogPageObjectResponse,
                    dependencies=[Depends(CheckUserInterfaceAuth('monitor:job:list'))])
...
@jobController.post("/jobLog/delete", response_model=CrudJobResponse,
                    dependencies=[Depends(CheckUserInterfaceAuth('monitor:job:remove'))])
...
@jobController.post("/jobLog/clear", response_model=CrudJobResponse,
                    dependencies=[Depends(CheckUserInterfaceAuth('monitor:job:remove'))])
...
@jobController.get("/jobLog/{job_log_id}", response_model=JobLogModel,
                   dependencies=[Depends(CheckUserInterfaceAuth('monitor:job:query'))])
...
async def export_system_job_log_list(request: Request, job_log_query: JobLogQueryModel,
    ...
    try:
        ...
        logger.log_info('导出成功')
        return streaming_MyResponse(data=bytes2file_response(job_log_export_result))
    except Exception as e:
        logger.exception(e)
        return MyResponse(data="", msg=str(e))
