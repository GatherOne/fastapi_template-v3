...
from app.service.captcha_service import *
...
async def get_captcha_image(request: Request):
    try:
        ...
        logger.log_info(f'编号为{session_id}的会话获取图片验证码成功')
        return MyResponse(data={'image': image, 'session_id': session_id}, msg='获取验证码成功')
