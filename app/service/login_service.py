...
async def get_sms_code_services(request: Request, result_db: Session, user: ResetUserModel):
    ...
    if redis_sms_result:
        return SmsCode(**dict(is_success=False, sms_code='', session_id='', msg='短信验证码仍在有效期内'))
    ...
    if is_user:
        ...
        return SmsCode(**dict(is_success=True, sms_code=sms_code, session_id=session_id, msg='获取成功'))

    return SmsCode(**dict(is_success=False, sms_code='', session_id='', msg='用户不存在'))
