...
class OperationLogService:
    ...
    def delete_operation_log_services(cls, result_db: Session, page_object: DeleteOperLogModel):
        ...
        if page_object.oper_ids.split(','):
            ...
            result = dict(is_success=False, msg='传入操作日志id为空')
        return CrudLogResponse(**result)
    ...
    def clear_operation_log_services(cls, result_db: Session, page_object: ClearOperLogModel):
        ...
        if page_object.oper_type == 'clear':
            try:
                ...
                result_db.commit()
                result = dict(is_success=True, msg='清除成功')
            except Exception as e:
                result_db.rollback()
                result = dict(is_success=False, msg=str(e))
        else:
            result = dict(is_success=False, msg='清除标识不合法')
...
class LoginLogService:
    ...
    def delete_login_log_services(cls, result_db: Session, page_object: DeleteLoginLogModel):
        ...
        if page_object.info_ids.split(','):
            ...
            result = dict(is_success=False, msg='传入登录日志id为空')
        return CrudLogResponse(**result)
    ...
    async def unlock_user_services(cls, request: Request, unlock_user: UnlockUser):
        ...
        if locked_user:
            ...
            result = dict(is_success=True, msg='解锁成功')
        else:
            result = dict(is_success=False, msg='该用户未锁定')
        return CrudLogResponse(**result)
