...
from app.service.log_service import OperationLogService, LoginLogService
from app.entity.vo.log_vo import OperLogModel, LogininforModel
...
def log_decorator(title: str, business_type: int, log_type: Optional[str] = 'operation'):
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            json_result = json.dumps(dict(code=result_dict.get('code'), msg=result_dict.get('message')), ensure_ascii=False)
            # 根据响应结果获取响应状态及异常信息
            status = 1
            error_msg = ''
